...
bool Mesh::loadMeshFromOBJ(const char * const objFile, float scale, bool inverseTexture) 
{
	...
	vertexArray.reserve(vIndex.size() * 8);
	...
	return m_shapeBuffer.loadVertexBuffer((float*)&vertexArray[0], vertexArray.size() / 8, 8);
}
//--------------------------------------------------------------------