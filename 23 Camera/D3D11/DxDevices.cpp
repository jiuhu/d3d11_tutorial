...
bool DxDevices::init(HWND hWnd)
{
	...
	hr = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags, levels, ARRAYSIZE(levels), 
	...
	if(FAILED(hr))
	{
		printErrorMessage(hr, "m_swapChain->GetBuffer"); 
        return false;
	}	
	...
	m_device11->CreateDepthStencilState(&dsd, &m_depthStencilState);
	...
	m_device11->CreateRasterizerState(&rd, &m_rasterizerState);
...
void DxDevices::cullMode(D3D11_CULL_MODE mode)
{
	D3D11_RASTERIZER_DESC rd;
...
void DxDevices::polygonMode(D3D11_FILL_MODE mode)
{
	D3D11_RASTERIZER_DESC rd;
	m_rasterizerState->GetDesc(&rd);
	rd.FillMode = mode;
...
void DxDevices::toggleDepth(bool depthOn)
{
	D3D11_DEPTH_STENCIL_DESC dsd;
